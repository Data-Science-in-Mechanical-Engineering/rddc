...
plot_file_name = 'exp_pos_'+session_name + '_' + weight_configuration + fan_configuration
...
positions, times = get_time_series(abs_traj_paths)
...
for trajId in range(len(positions)):
    x = positions[trajId][0,:]
    y = positions[trajId][1,:]
    t = times[trajId]
...
ax1.set(ylabel=('position x [m/s]'))
ax2.set(xlabel=('time [s]'))
ax2.set(ylabel=('position y [m/s]'))
