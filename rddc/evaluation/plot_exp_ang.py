...
def get_time_series(paths):
    ...
    for (value, time) in zip(values, times):
        idx_nonzero = np.logical_and(np.abs(value[6,:]) > 1e-6, np.abs(value[7,:]) > 1e-6)
...
plot_file_name = 'exp_ang_'+session_name + '_' + weight_configuration + fan_configuration
...
angles, times = get_time_series(abs_traj_paths)
...
for trajId in range(len(angles)):
    roll = angles[trajId][6,:]*180/np.pi
    pitch = angles[trajId][7,:]*180/np.pi
    t = times[trajId]
    ax1.plot(t,roll,'-',linewidth=0.5, color=deep_sns_colors[trajId], zorder=2)
    ax2.plot(t,pitch,'-',linewidth=0.5, color=deep_sns_colors[trajId], zorder=2)
    if outcomes[trajId]:
        ax1.scatter([t[-1]], [roll[-1]], marker='.', s=5, linewidths=1, color=deep_sns_colors[trajId])
        ax2.scatter([t[-1]], [pitch[-1]], marker='.', s=5, linewidths=1, color=deep_sns_colors[trajId])
    else:
        ax1.scatter([t[-1]], [roll[-1]], marker='x', s=5, linewidths=1, color=deep_sns_colors[trajId])
        ax2.scatter([t[-1]], [pitch[-1]], marker='x', s=5, linewidths=1, color=deep_sns_colors[trajId])
# trajLine, = ax.plot([],[],'-k',linewidth=0.8,)
# refLine, = ax.plot(ref[:, 0],ref[:, 1],'--k',linewidth=1.5, zorder=2)
# ax.axis('equal')
ax1.set(ylim = (-12.5, 12.5))
ax2.set(ylim = (-12.5, 12.5))
# ax.set(xlim = (-1.05, 2))
...
ax1.set(ylabel=('roll [deg]'))
ax2.set(xlabel=('time [s]'))
ax2.set(ylabel=('pitch [deg]'))
...
fig.subplots_adjust(bottom=0.18, top=0.98, left=0.18, right=.95)
