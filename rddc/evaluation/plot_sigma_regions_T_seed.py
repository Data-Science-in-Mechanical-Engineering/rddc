...
(fig_width_in, fig_height_in) = evaltools.get_size(400, subplots=(1,1), fraction=1)
fig, axes = plt.subplots(figsize=(fig_width_in, fig_height_in*1.6), nrows=len(Ts), ncols=len(seeds))
...
for row in range(len(Ts)):
    ...
    for col in range(len(seeds)):
        ...
        if col > 0:
            ...
            ax.set_ylabel(r'$T = {}$'.format(Ts[row]), rotation=0, ha='right')
            ax.yaxis.set_label_coords(-1.3, 0.4)
        if row < len(Ts)-1:
            ax.set_xticklabels([])
        ax.set_xlim([0.6, 1.2])
        ax.set_ylim([1.1, 1.7])
...
fig.legend(
    ...
    loc=[0.2,0.92],
    borderaxespad=0., ncol=2, columnspacing=7.0
)
fig.text(0.15, 0.5, r'A')
fig.text(0.6, 0.01, r'B')
# ax.annotate("",
...
fig.subplots_adjust(bottom=0.08, top=0.9, left=0.25, right=.95)
